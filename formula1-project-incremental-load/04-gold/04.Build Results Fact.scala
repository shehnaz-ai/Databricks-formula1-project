...
val target_table =catalog_name + "." + gold_schema +".fact_session_results"
...
val results_df = (
    spark.table(catalog_name+"." + silver_schema+".results")
...
val sprints_df = (
    spark.table(catalog_name+"." + silver_schema+".sprints")
         .filter(col("batch_id") === v_batch_id)
...
val results_sprints_df = results_df.unionByName(sprints_df)
